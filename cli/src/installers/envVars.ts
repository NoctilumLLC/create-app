...
export const envVariablesInstaller: Installer = ({
  ...
  if (usingDb) {
    if (usingPlanetScale) {
      if (usingNextAuth || usingWorkOS) envFile = "with-auth-db-planetscale.js";
      else envFile = "with-db-planetscale.js";
    } else {
      if (usingNextAuth || usingWorkOS) envFile = "with-auth-db.js";
  ...
  let _envContent = envContent;

  // Generate secrets and put in .env, not .env.example
  const secret = Buffer.from(
    crypto.getRandomValues(new Uint8Array(32))
  ).toString("base64");

  if (usingNextAuth) {
    _envContent = _envContent.replace(
      'AUTH_SECRET=""',
  ...
  if (usingWorkOS) {
    _envContent = _envContent.replace(
      ...
      `WORKOS_COOKIE_PASSWORD="${secret}" # Generated by create-t3-app.`
    );
  }
...
`;
  ...
  if (usingWorkOS)
    content += `
# WorkOS AuthKit
# Get your API key and Client ID from https://dashboard.workos.com/
WORKOS_API_KEY=""
WORKOS_CLIENT_ID=""
WORKOS_COOKIE_PASSWORD=""
...
DATABASE_URL='mysql://YOUR_MYSQL_URL_HERE?sslaccept=strict'`;
  ...
  if (!usingNextAuth && !usingWorkOS && !usingPrisma && !usingDrizzle)
    content += `
# Example:
# SERVERVAR="foo"

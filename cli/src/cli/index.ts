...
export const runCli = async (): Promise<CliResults> => {
  ...
  const program = new Command()
    ...
    .option(
      "--workos [boolean]",
      "Experimental: Boolean value if we should install WorkOS AuthKit. Must be used in conjunction with `--CI`.",
      (value) => !!value && value !== "false"
    )
    /** @experimental - Used for CI E2E tests. Used in conjunction with `--CI` to skip prompting. */
    .option(
  ...
  if (cliResults.flags.CI) {
    ...
    if (cliResults.flags.nextAuth && cliResults.flags.workos) {
      logger.warn("Incompatible combination NextAuth + WorkOS. Exiting.");
      process.exit(0);
    }
  ...
  can provide the arguments from the CLI directly: https://create.t3.gg/en/installation#experimental-usage to skip the prompts.`);
    ...
    if (project.authentication === "workos") packages.push("workos");
